...
const context={window:{}};vm.runInNewContext(source,context);const mobile=context.window.PhysMobile3D;
...
for(const kind of ['fly','butterfly','ladybug'])for(const time of [0,.125,.25,2.4]){
...
(async()=>{
 ...
 try{
  ...
  const metrics=await page.evaluate(()=>{
    ...
    for(const kind of ['fly','butterfly','ladybug'])for(const [label,eyeUnit,right,up,distance]of views){
  ...
  for(const kind of ['fly','butterfly','ladybug']){
    ...
    assert(front.height<top.height,kind+' heading foreshortens');
  }
  ...
  await page.selectOption('#mobile-appearance','ladybug');
  ...
  for(const preset of ['top-view','front-view','side-view','reset-view']){await page.locator('#'+preset).click();assert.deepEqual(await page.evaluate(()=>window.__pose),pose,'camera must not rotate the insect physically');}
  ...
  await page.locator('#viewport').screenshot({path:path.join(out,'app-perspective-ladybug.png')});
