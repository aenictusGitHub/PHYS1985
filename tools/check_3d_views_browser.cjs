...
(async()=>{
 ...
 try{
  ...
  const differentialGlyphs=await page.locator('.jerk-relation .equation svg [data-c]').evaluateAll(es=>es.map(e=>e.getAttribute('data-c')));
  assert.equal(differentialGlyphs.filter(c=>c==='1D451').length,4,'The four differential d symbols are italic');
  assert(!differentialGlyphs.includes('64'),'No upright differential d');
