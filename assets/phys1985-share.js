...
(() => {
  ...
  function check(x, depth = 0, budget = {n: 0}) {
    if (++budget.n > 600000 || depth > 18) throw Error('Configuration trop volumineuse.');
    if (x === null || typeof x === 'boolean') return;
    if (typeof x === 'number') { if (!Number.isFinite(x) || Math.abs(x) > 1e30) throw Error('Nombre non valide.'); return; }
    if (typeof x === 'string') { if (x.length > 3000) throw Error('Texte trop long.'); return; }
    if (!x || typeof x !== 'object') throw Error('Configuration non valide.');
    for (const k of Object.keys(x)) {
      if (['__proto__', 'prototype', 'constructor'].includes(k)) throw Error('Clé non autorisée.');
      check(x[k], depth + 1, budget);
    }
  }
  function set(target, source) {
    if (!source || typeof source !== 'object' || Array.isArray(source)) throw Error('Objet attendu.');
    for (const [k, v] of Object.entries(source)) {
      if (!Object.hasOwn(target, k) || typeof v !== typeof target[k]) throw Error('Paramètre non reconnu : ' + k);
      if (v && typeof v === 'object' && !Array.isArray(v)) set(target[k], v);
      else target[k] = clone(v);
    }
  }
  function range(x, min, max) { if (typeof x !== 'number' || !Number.isFinite(x) || x < min || x > max) throw Error('Valeur hors limites.'); return x; }
  function member(x, options) { if (!options.includes(x)) throw Error('Choix non reconnu.'); return x; }
  function controls() {
    return Object.fromEntries([...document.querySelectorAll('input[id],select[id]')]
      .filter(e => !e.closest('[data-share-ui]') && !['file', 'password', 'button', 'submit'].includes(e.type))
      .map(e => [e.id, ['checkbox', 'radio'].includes(e.type) ? e.checked : e.value]));
  }
  function applyControls(values) {
    for (const [id, v] of Object.entries(values || {})) {
      const e = document.getElementById(id);
      if (!e || e.closest('[data-share-ui]') || !e.matches('input,select')) continue;
  ...
  async function streamBytes(stream) {
    ...
    for (;;) { const {done, value} = await r.read(); if (done) break; size += value.length; if (size > MAX_BYTES) { await r.cancel(); throw Error('Configuration trop volumineuse.'); } chunks.push(value); }
  ...
  async function encode(snapshot) {
    ...
    if (bytes.length > MAX_BYTES) throw Error('Historique trop long pour un lien. Recommencez une expérience plus courte.');
  ...
  async function decode(encoded) {
    if (encoded.length > MAX_BYTES * 1.4 || !/^1[zj]\.[A-Za-z0-9_-]+$/.test(encoded)) throw Error('Lien incomplet ou format non reconnu.');
  ...
  function capture() {
    if (!ready) throw Error('L’application se prépare encore.');
  ...
  async function restore(snapshot) {
    check(snapshot);
    if (snapshot.version !== 1 || snapshot.app !== adapter.id || !snapshot.data || !snapshot.controls) throw Error('Ce lien concerne une autre application ou une version non prise en charge.');
  ...
  async function makeLink() {
    ...
    const url = new URL(location.href); url.search = ''; url.hash = PREFIX + await encode(snapshot); return url.href;
  }
  ...
  function mount() {
    ...
    const button = document.createElement('button'); shareButton=button;button.disabled=true;button.type = 'button'; button.id = 'share-configuration'; button.textContent = 'Partager'; button.title = 'Créer un lien conservant paramètres, options, vue et instant — ouvert en pause';
    panel = document.createElement('div'); panel.className = 'phys-share-panel'; panel.hidden = true;
    const label = document.createElement('label'); label.htmlFor = 'share-configuration-link'; label.textContent = 'Lien vers cet état, ouvert en pause';
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        message((copied ? 'Lien copié.' : 'Copiez le lien ci-dessus.') + (local ? ' Lien local : nécessite ce même fichier ou serveur. La version publiée permettra le partage aux étudiants.' : ' La configuration s’ouvrira en pause.') + (url.length > 16000 ? ' Lien long : certains outils de messagerie peuvent le tronquer.' : ''));
  ...
  async function restoreLocation() {
    ...
    try {
      ...
      await restore(snapshot);if(!panel.hidden)linkField.value=location.href;message('Configuration restaurée — en pause.');return true;
    } catch (error) {if(sequence===restoreSequence)message('Lien non chargé : ' + error.message, true);return false;}
  }
  async function register(a) {
    if (adapter) throw Error('Une seule application par page.');
    adapter = a; mount(); ready = true;
    window.addEventListener('hashchange',()=>{void restoreLocation();});
    try {return await restoreLocation();}
    finally {loaded=true;shareButton.disabled=false;}
  }
